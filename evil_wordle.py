...
On my/our honor, <Marc Betancourt> and <Bryan Simmons>, this
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    tested = []  # list that holds letters already seen
    for i in range(NUM_LETTERS):
        if secret_word[i] == guessed_word[i]:
            feedback[i] = CORRECT_COLOR
            tested.append(secret_word[i])
        elif guessed_word[i] not in secret_word:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for i in range(len(remaining_secret_words)):
        ...
        feedback_colors = tuple(feedback_colors)
        if feedback_colors not in families:  # makes the list of words for given pattern
            families[feedback_colors] = [remaining_secret_words[i]]
        else: # adds word with the same pattern to the list of words that has same
            families[feedback_colors].append(remaining_secret_words[i])
